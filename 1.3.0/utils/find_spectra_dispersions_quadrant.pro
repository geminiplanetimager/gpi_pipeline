...
pro find_spectra_dispersions_quadrant, quad,peakwavelen,apprXpos0,apprYpos,nlens,w3,w3med,tilt,specpos,im,wx,wy,hh,szim,edge_x1,edge_x2,edge_y1,edge_y2,dispeak,dispeak2,tight_tilt,meth=meth,statuswindow=statuswindow,progscale=progscale
...
;;if we're given a statuswindow, we're expected to update a progressbar
if keyword_Set(statuswindow) then begin
   totiter = (nlens/2 +1d)^2d ;; total number of iterations you'll do
   if ~keyword_set(progscale) then progscale = 0.25/2d
endif 
...
;            ;if it fails to converge then use an other method
            ...
            if specpos[nlens/2+i,nlens/2+j,0] eq -1 then begin
                ...
                pospeak=localizepeak( im, specpos[nlens/2+i,nlens/2+j,0]+apprXpos[p],specpos[nlens/2+i,nlens/2+j,1]+apprYpos[p],wx,wy,hh,meth="barycentric")
             endif
...
;                     specpos[nlens/2+i-3,nlens/2+j,4],specpos[nlens/2+i,nlens/2+j-3*jdir,4]]
    ...
    if (i eq 0) then begin
       ...
       if keyword_set(statuswindow) && obj_valid(statuswindow) then begin
          statuswindow->set_percent,-1,1d/totiter*100d*progscale,/append
       endif 
