...
fileSpectra='E:\DST'+path_sep()+'pickles'+path_sep()+'uk'+spect+'.dat'
...
function PIP_nbphot_trans, hdr, lambda, atmostrans=atmostrans, filtertrans=filtertrans
	;common DST_INPUT
...
filter_trans=gpi_getfilter(lamb, filter=filter) else filter_trans=1.
;	;sky background;
;	;==============================================
...
locg=intarr(2)
	...
	if i eq nlambda-1 then locg[1]=loc[1]
		...
		GroundSpec[i] = (1./((Lamb(2)-Lamb(1))*(dummy2-dummy)))*INT_TABULATED(Lamb(dummy:dummy2),HiResGrndSpec(dummy:dummy2),/DOUBLE)
	endfor
