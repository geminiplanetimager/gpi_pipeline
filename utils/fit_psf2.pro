...
; - first_guess, 3 by nz 2d array with
...
;     PSF_template = {values: fltarr(nx,ny), $          2D array with the ePSF values.
;                    xcoords: fltarr(nx), $             a vector with the x coordinates for the (assumed regular) grid of the psf.
;                    ycoords: fltarr(ny), $             a vector with the y coordinates for the (assumed regular) grid of the psf.
;                    tilt: 0.0,$
;                    id: [0,0,0,0] }                    a 4 elements vector containing indexes to locate the psf. in the context of the gpi pipeline. id[0] and id[1] are the indexes for the lenslet array. id[2] the spot number in case of polarimetry fata for example. id[3] is gonna be used for flexure management (not yet done). 
...
; 
; OUTPUTS:  
; - Returned value: a pointer to a cube of the same dimension as pixel_array with the fitted PSFs. return f_nan if an error occured (It should return the null pointer now but not done yet).
; 
...
;     -4: first_guess number of elements is not consistent with pixel_array third dimension.
;     -3: first_guess first dimension should have 3 elements.
;     -2: first_guess array number of dimensions is not right. Should be a 2d array.
...
;- 
function fit_PSF2, pixel_array, first_guess, $
                  X0 = x0, Y0 = y0, $
...
if (where(finite(psf)))[0] ne -1 then begin
  ...
  initialize_psf_interp, psf, x_grid_PSF, y_grid_PSF, 1
