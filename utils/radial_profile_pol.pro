pro radial_profile_pol,im0,cens,lambda=lambda,cent=cent,rmax=rmax,rsum=rsum,$  ;;inputs
  ...
  ;                19/03/2015 - Branched off for pol mode. MMB
  ;-
  ...
  sz = size(im0,/dim)
  if not keyword_Set(cent) then cent = [mean(cens[0,*]),mean(cens[1,*])] ;The center of the PSF
  ...
  ;For now skip the masking of the satellite spots. 
  ;Eventually we will want to mask this out using the racetrack apertures 
