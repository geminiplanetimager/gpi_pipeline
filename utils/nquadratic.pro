...
;    start_params[9]-quadratic term
;    start_params[10-end]-flux ratios
;    
;+

FUNCTION nquadratic,x,y, start_params
...
for peak = 0, numgauss-1 do begin
   lambda=wl[peak]
   coeff=start_params[10+peak]
   xcent=xo+sin(theta)*((lambda-lambdao)/w+(lambda-lambdao)^2*quad)
   ycent=yo-cos(theta)*((lambda-lambdao)/w+(lambda-lambdao)^2*quad)
