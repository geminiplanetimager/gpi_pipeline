...
;- 
function get_spaxels2, my_type, image, calibration, width, STAMPS_MODE = stamps_mode;, CENTROID_MODE = centroid_mode
...
case my_type of
  'PRISM': begin      
      ...
      sdpx = calc_sdpx(wavcal, filter, xmini, CommonWavVect) ; get the length of the spectrum (spdx)
...
; need an spdx that is large enough for the entire field 
...
; we pretty much always run in stamps mode, so this is moot
...
if tmpy-ycentroids[i,j,0,it_elev] ge 1.1*tmpy then stop
              ...
              mask_image[xcoords[where(finite(xcoords[*,i,j])),i,j],ycoords[where(finite(ycoords[*,i,j])),i,j]] = 1
        ; this xlimit is different... not sure why... 
              xlim = round(wavcal[i,j,1]+(wavcal[i,j,0]-xmini[i,j])*tan(wavcal[i,j,4]) + float((width-1))/2.)
              ylim = xmini[i,j]-ny+1
