...
if (Q12_ind ne -1 and Q22_ind ne -1) then begin
	...
	Rx2=( (xgrid[Q22_ind]-0.0)/(xgrid[Q22_ind]-xgrid[Q12_ind]) )*Q22_ind_psf + $
			( (0.0-xgrid[Q12_ind])/(xgrid[Q22_ind]-xgrid[Q12_ind]) )*Q12_ind_psf
; TO DO!
...
if (Q11_ind ne -1 and Q21_ind ne -1) then begin
	...
	; note that no interpolation is ever necessary since they all have the same
	; grid spacing
	Q21_ind_psf=(*high_res_psfs[Q21_ind mod 281,Q21_ind / 281]).values
...
Rx1_avg_y=(ygrid[Q21_ind]+ygrid[Q11_ind])/2.0
	...
	new_psf= ( (Rx2_avg_y-0.0)/(Rx2_avg_y-Rx1_avg_y) )*(Rx2) + $
					( (0.0-Rx1_avg_y)/(Rx2_avg_y-Rx1_avg_y) )*(Rx1)
