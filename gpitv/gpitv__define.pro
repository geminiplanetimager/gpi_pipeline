...
pro GPItv::open, input1, input2, _extra=_Extra
  ...
  endelse
  if keyword_set(_extra) && (tag_exist(_extra,'dispwavecalgrid')) then begin
     (*self.state).wcfilename=_extra.dispwavecalgrid
...
pro GPItv::readfits, fitsfilename=fitsfilename, alignwav=alignwav, dispwavecalgrid=dispwavecalgrid, imname=imname, _extra=_extra
  ;; Parameters
  ;; 	fitsfilename	optional name of FITS file to read
  ;; 	alignwav		
  ;; 	dispwavecalgrid	Overplot a GPI wavecal grid on an image
  ;; 	image			?
  ;;
...
pro GPItv::contrprof_refresh, ps=ps,  sav=sav, radialsav=radialsav,forcesat=forcesat,forcecalc=forcecalc
