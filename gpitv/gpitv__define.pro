...
self.images.header_stack = ptr_new(/alloc)
...
state = {                   $
        ...
        cur_image_sticky: 1L, $ ; toggles stickiness of current image when loading new file
        ...
        filter1_id: 0L, $                   ; widget id for'FILTER1')
        ...
        smart: 0S, $                        ; whether GPItv is run in smart
...
; Can be re-instated if "WCS Grid" required but will need modification.
                ...
                {cw_pdmenu_s, 4, 'Select Wavcal Grid'}, $
                {cw_pdmenu_s, 0, 'Plot Wavcal Grid'}, $
...
tmp = gpi_get_setting('gpitv_cur_image_sticky',/silent,/int)
if (size(tmp,/type) eq 2) && ( (tmp eq 0) || (temp eq 1) ) then (*self.state).cur_image_sticky = tmp
widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Retain Current Slice")],$
                 set_button = (*self.state).cur_image_sticky

tmp = gpi_get_setting('gpitv_default_align',/silent,/int)
if (size(tmp,/type) eq 2) && ( (tmp eq 0) || (temp eq 1) ) then (*self.state).default_align = tmp
widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Auto Align")],$
                 set_button = (*self.state).default_align
...
tmp = gpi_get_setting('gpitv_default_stretch',/silent,/int)
if (size(tmp,/type) eq 2) && ( (tmp eq 0) || (temp eq 1) ) then (*self.state).default_stretch = tmp
widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Retain Current Stretch")],$
                 set_button = (*self.state).default_stretch
...
(*self.state).filter1_id = widget_label ((*self.state).info_head_id, $
                                      value = (*self.state).filter1,  $
                                      uvalue = 'filter1',  frame = 1, /DYNAMIC_RESIZE, /ALIGN_CENTER)
...
pro gpitv::cleanup
	...
	ptr_free, self.images.header_stack
...
case event_name of
   ...
   'Select Wavcal Grid':begin
      (*self.state).wcfilename= DIALOG_PICKFILE( TITLE='Select wavcal grid ', FILTER = '*.fits', FILE='*.fits',/MUST_EXIST )
      self->message, msgtype = 'information',  "Wav Cal grid file set to "+(*self.state).wcfilename 
   end
   'Plot Wavcal Grid': self->wavcalgridlabel
   ...
   'Rotate GPI field square': begin
		...
		if sz[1] eq 2048 and sz[2] eq 2048 then begin
		   ...
		   self->rotate, (-1) * gpi_get_setting('ifs_rotation_angle', default=24.5)
		endelse
	end
   ...
   'Retain Current Slice': BEGIN
      (*self.state).cur_image_sticky = 1 - (*self.state).cur_image_sticky
      widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Retain Current Slice")],$
                       set_button = (*self.state).cur_image_sticky
   END

   'Auto Align': begin
      (*self.state).default_align = 1 - (*self.state).default_align
      widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Auto Align")],$
                       set_button = (*self.state).default_align
   end
   ...
   "Retain Current Stretch": begin
      (*self.state).default_stretch = 1 - (*self.state).default_stretch
      widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Retain Current Stretch")],$
                       set_button = (*self.state).default_stretch
   end
   ...
   'Display Full Paths': begin
      (*self.state).showfullpaths = 1 - (*self.state).showfullpaths
      widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Display Full Paths")],$
                       set_button = (*self.state).showfullpaths
      self->settitle
   end
   ...
   'Suppress Information Messages': begin
      (*self.state).noinfo = 1 - (*self.state).noinfo
      widget_control,  (*self.state).menu_ids[ where((*self.state).menu_labels eq "Suppress Information Messages")],$
                       set_button = (*self.state).noinfo
   end
...
pro GPItv::collapsecube
  ...
  ;; if you were previously speckle aligned, restore the backup cube
  ...
  case modelist[(*self.state).collapse] of
     ...
     'Collapse by diff': begin
        widget_control,(*self.state).curimnum_base_id,map=0
                                ; TODO this is a quick demo of a very rough methane filter in H
                                ; band.  Make this much better!!!
        self->message,msgtype='warning', /window,"HARD-CODED DIFF for GPI H band Methane. Probably won't work right for whatever arbitrary cube you have! **FIXME**"
        szi=size((*self.images.main_image_stack))
        if szi[3] eq 37 then begin
           lim1=4 & lim2=10 & lim3=13 & lim4=19
        endif else begin
           lim1=szi[3]/4 & lim2=szi[3]/4+4 & lim3=szi[3]/2 & lim4=szi[3]/2+4
        endelse
        *self.images.main_image=total((*self.images.main_image_stack)[*,*,lim1:lim2],3,/NAN) -$
                                total((*self.images.main_image_stack)[*,*,lim3:lim4],3,/NAN)
        if bpct gt 0 then (*self.images.main_image)[wn] = !values.f_nan
     end
...
;-----------------------------------------------------
pro GPItv::invert, ichange
; Routine to do image axis-inversion (X,Y,X&Y)
;  With FITS header astrometry update too. 
;
;
; Now that main_image_backup exists, this code could be much simplified, I
; think, by just always recalculating from that? Although then you couldn't invert a derived image like a difference
; image. So, leave the code as is for now. 
...
pro GPItv::rotate, desired_angle, get_angle=get_angle
...
; and applying the same transformation to the min/max display scaling.
   ...
   if n_elements((*self.state).image_size) eq 3 then nim = ((*self.state).image_size)[2] else nim = 1
	...
	; special case: the easy conversions from ADU <-> ADU/s
	...
	if (*self.state).current_units eq 'ADU per coadd' and new_requested_units eq 'ADU/s' then begin
		...
		; here, we deal with the nontrivial conversions that require flux
		; calibration
	...
	*self.images.main_image_stack *= conversion_factor
	...
	if (*self.state).CWV_NLam gt 0 then *self.images.main_image = (*self.images.main_image_stack)[*, *, (*self.state).cur_image_num]

	self->getstats,/noerase ; updates image min/max stats displayed on screen
	self->set_minmax ; updates the min/max textboxes for display stretch
	self->displayall ; does the actual display
...
pro GPItv::changeimage,imagenum,next=next,previous=previous, nocheck=nocheck,$
	...
	if ((*self.state).collapse ne 0) && ((*self.state).specalign_mode ne 1) then return
...
;   2011-07-29 MP: Algorithm reworked to work more reliably. 
	...
	while (((*self.state).image_size[0] * (*self.state).zoom_factor gt (*self.state).draw_window_size[0]) AND $
	           ((*self.state).image_size[1] * (*self.state).zoom_factor gt (*self.state).draw_window_size[1]) and max_zooms gt 0) do begin
	   ;print, 'autozoom out', (*self.state).image_size[0]*2, (*self.state).draw_window_size[0]
	   self->zoom, 'out';, /recenter
	   max_zooms -= 1
	endwhile

	while ( ((*self.state).image_size[0]*2 * (*self.state).zoom_factor le (*self.state).draw_window_size[0]) AND $
           ((*self.state).image_size[1]*2 * (*self.state).zoom_factor le (*self.state).draw_window_size[1])  and max_zooms gt 0) do begin
	   ;print, 'autozoom in', (*self.state).image_size[0]*2, (*self.state).draw_window_size[0]
	   self->zoom, 'in';,/recenter
	   max_zooms -= 1
	endwhile
...
offset=6 ;could be platform dependent, works on Windows platform
...
; use for debugging
; print, 'self->setwindow', (*self.state).active_window_id
...
;
;  Keywords:
;     imname    string for overriding the filename in the window title bar,
;             if desired.
...
pro GPItv::open, input1, input2, _extra=_Extra
  ...
  if size(input1,/TNAME) eq 'STRING' then begin
     ...
     *self.images.main_image = input1
     self->setup_new_image, header=input2, _extra=_extra
  endelse
  if keyword_set(_extra) && (tag_exist(_extra,'dispwavcalgrid')) then begin
     (*self.state).wcfilename=_extra.dispwavcalgrid
     self->wavcalgrid, gridcolor=1, tiltcolor=2, labeldisp=0,  labelcolor=7, charsize=1.0, charthick=1
  endif 
end

;--------------------------------------------------------------------

pro GPItv::readfits, fitsfilename=fitsfilename, alignwav=alignwav, dispwavcalgrid=dispwavcalgrid, imname=imname, _extra=_extra
  ;; Parameters
  ;; 	fitsfilename	optional name of FITS file to read
  ;; 	alignwav		
  ;; 	dispwavcalgrid	Overplot a GPI wavecal grid on an image
  ;; 	image			?
  ;;
...
;--------------------------------------------------------------------
pro GPItv::setup_new_image, header=header, imname=imname, keepstretch=keepstretch, _extra=_extra
  ...
  ;; Keywords:
  ;;
  ;;   keepstretch : use the same stretch min/max when opening the new file
  ...
  ;; First, based on the new image, set all the metadata such 
  case (size(*self.images.main_image))[0] of
     ...
     3: begin                   ; case of 3-d imagecube
        ...
        *self.images.main_image = (*self.images.main_image_stack)[*, *, 0]
        ...
        ;;slice
        if ((*self.state).firstimage eq 1) || ((*self.state).cur_image_sticky eq 0) then $
  ...
  *self.satspots.asec = ptrarr((*self.state).image_size[2],/alloc)        ;arrays of ang sep vals
  ...
  if (*self.state).firstimage then begin
     self->recenter
     self->autoscale
     ;;if (*self.state).autozoom then self->autozoom  
  endif

  if ~(keyword_set(keepstretch)) then self->getstats
  self->recenter, align=(*self.state).default_align
  self->settitle
  if ((*self.state).default_align EQ 0) then begin
     (*self.state).zoom_level =  0
     (*self.state).zoom_factor = 1.0
  endif
  if ((*self.state).default_stretch EQ 0) &&  ((*self.state).default_autoscale EQ 1) && $
     ~(keyword_set(keepstretch)) then self->autoscale
  self->setheadinfo,_extra=_extra
  self->set_minmax
  self->collapsecube
  self->setcubeslicelabel
  self->displayall
  if (*self.state).firstimage then self->autozoom
  (*self.state).firstimage = 0

  ;;Reset image rotation angle to 0 and inversion to none
  (*self.state).rot_angle = 0.
  (*self.state).invert_image = 'none'

  if keyword_set(dispwavcalgrid) && ((size(*self.images.main_image))[0] EQ 2) then begin
     (*self.state).wcfilename=dispwavcalgrid
     self->wavcalgrid, gridcolor=1, tiltcolor=2, labeldisp=0,  labelcolor=7, charsize=1.0, charthick=1
  endif
...
pro GPItv::update_sat_spots,locs0=locs0
  ...
  *self.satspots.satflux = sats ;;4 x Z array of sattellite spot fluxes
...
IF (filename EQ (*self.state).current_dir) then BEGIN
...
fname = strmid(fname,strpos(fname,'/',/reverse_search)+1,$
               strpos(fname,'.fits')-strpos(fname,'/',/reverse_search)-1)
...
formdesc = ['0, droplist, JPEG|TIFF|BMP|PICT|PNG,label_left=FileFormat:,set_value=4, TAG=format ', $
            ...
            '2, text, '+fname+', width=50, TAG=filetext', $
            '1, base, , row', $
...
CASE event.tag OF
    ...
    'FILE': BEGIN
        ...
        fname = dialog_pickfile(/write, file=fname,path=path,$
                                default_extension=dext,filter='*'+dext,/fix_filter)
        if strcmp(fname,'') then return
        widget_control,(*(*self.state).makemovie_ids_ptr)[2], set_value=fname
...
formdesc = ['0, droplist, GIF|MPEG|PNG, label_left=FileFormat:,set_value='+strtrim(formatdef,2)+', TAG=format ', $
            '1, base, , row', $
            '0, button, Choose..., TAG=file ', $
            '2, text, '+fname+', width=40, TAG=filetext', $
            '1, base, , row', $
...
if cc gt 0 then (*self.state).obsfilt=strcompress(val,/REMOVE_ALL)
...
if cce gt 0 then begin
    (*self.state).itime = itime
...
if naxis eq 3 then begin
    ...
    case mode of
        'WAVE': begin
            ...
            modelist = ['Show Cube Slices', 'Collapse by Mean', 'Collapse by Median', 'Collapse by diff', 'Collapse by SDI', 'Collapse to RGB Color','Align speckles']
...
pro GPItv::switchextension
	...
	self->setup_new_image, header=head, imname=(*self.state).imagename, _extra=_extra, /keepstretch
...
for iplot = 1, self.pdata.nplot do begin
    case (*(self.pdata.plot_ptr[iplot])).type of
        ...
        'wavcalgrid' : self->plot1wavcalgrid, iplot
...
pro GPItv::wavcalgrid, _extra = options

; Routine to read in wavcal overplot options, store in a heap variable
; structure, and overplot the grid
...
pstruct = {type:'wavcalgrid', $            ; type of plot
           options: options $           ; plot keyword options
          }
...
self->plotwindow
self->plot1wavcalgrid, self.pdata.nplot
...
;----------------------------------------------------------------------
pro GPItv::plot1wavcalgrid,iplot
...
wavcal=readfits((*self.state).wcfilename, ext=n_ext,/SILENT)
...
wavcal=readfits((*self.state).wcfilename, ext=1,/SILENT)
	for jj=0,sz(2)-1 do begin
		...
		X[0,*] = (float(wavcal(0,jj,*,0)) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
		X[1,*] = (float(wavcal(1,jj,*,0)) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor
		; and the rows
		X2[0,*] = (float(wavcal(0,*,jj,0)) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
		X2[1,*] = (float(wavcal(1,*,jj,0)) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor

		; Polarization 2
		; grab the X and Y locations of the columns
		P[0,*] = (float(wavcal(0,jj,*,1)) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
		P[1,*] = (float(wavcal(1,jj,*,1)) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor
		; and the rows
		P2[0,*] = (float(wavcal(0,*,jj,1)) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
		P2[1,*] = (float(wavcal(1,*,jj,1)) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor
	...
	if ((size(wavcal))[3] ne 5) then begin
		self->message, msgtype = 'warning', 'Selected wavcal file has invalid array axes lengths. Can not plot.'
		return
	endif
	for jj=0,sz(1)-1 do begin
		...
		;wnz = where( wavcal[*,jj,0]+wavcal[*,jj,1] ne 0 )
		X2[0,*]=(float(wavcal[*,jj,1]) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
		X2[1,*]=(float(wavcal[*,jj,0]) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor
		X[0,*]=(float(wavcal(jj,*,1)) - (*self.state).offset[0] + 0.5) * (*self.state).zoom_factor
		X[1,*]=(float(wavcal(jj,*,0)) - (*self.state).offset[1] + 0.5) * (*self.state).zoom_factor
		for ii=0,sz[1]-1 do begin
			;if (wavcal(ii,jj,0)+wavcal(ii,jj,1) ne 0) then begin ;&& (wavcal(ii+1,jj,0)+wavcal(ii+1,jj,0) ne 0) then begin
				;X2[0,ii]=(float(wavcal[ii,jj,1]) - (*self.state).offset[0] + 0.5) *  (*self.state).zoom_factor
				;X2[1,ii]=(float(wavcal[ii,jj,0]) - (*self.state).offset[1] + 0.5) *  (*self.state).zoom_factor
			;endif
			;if (wavcal(jj,ii,0)+wavcal(jj,ii,1) ne 0) then begin ;&& (wavcal(ii+1,jj,0)+wavcal(ii+1,jj,0) ne 0) then begin
				;X(0,ii)=(float(wavcal(jj,ii,1)) - (*self.state).offset[0] + 0.5) * (*self.state).zoom_factor
				;X(1,ii)=(float(wavcal(jj,ii,0)) - (*self.state).offset[1] + 0.5) * (*self.state).zoom_factor
			;endif

			if  wavcal(ii,jj,3) ne 0 then begin
...
;X2=fltarr(2,sz(1))+!VALUES.F_NAN
;	for jj=0,sz(1)-1 do begin
;		if (wavcal(ii,jj,0)+wavcal(ii,jj,1) ne 0) then begin ;&& (wavcal(ii,jj+1,0)+wavcal(ii,jj+1,1) ne 0) then begin
;		X2(0,jj)=(float(wavcal(ii,jj,0)) - (*self.state).offset[0] + 0.5) * $
;                   (*self.state).zoom_factor
;		X2(1,jj)=(float(wavcal(ii,jj,1)) - (*self.state).offset[1] + 0.5) * $
;                   (*self.state).zoom_factor
;        endif
;	endfor
; plots,X2,/DEVICE
;endfor
;GPItvplot,[wavcal(95,6,0),wavcal(95,6+1,0)],[wavcal(95,6,1),wavcal(95,6+1,1)],color = 1
...
pro GPItv::wavcalgridlabel

; Front-end widget for Wavcal labels
...
if (*self.state).multisess GT 0 then title_base = "GPItv #"+strc((*self.state).multisess) else title_base = 'GPItv '
gridform=cw_form(formdesc, /column, title = title_base+' Wavcal Grid')
...
self->wavcalgrid, gridcolor=gridform.tag0, tiltcolor=gridform.tag1, labeldisp=gridform.tag2, $
...
;; currently in the min/max boxes
  ...
  cents = mean(*self.satspots.cens,dim=2)
...
; before pressing 'Create PS' button
  ...
  shade_surf, shade_image, $
    ...
    color = 7, charsize=1.5;, ax=60, az=60
...
h[i] =  '**************************************************'
i = i + 1
h[i] =  '* This is only a very brief, incomplete list of  *'
i = i + 1
h[i] =  '* instructions. Please see the full GPItv user   *'
i = i + 1
h[i] =  '* manual on the web.                             *'
i = i + 1
h[i] =  '**************************************************'
i = i + 1
h[i] =  ''
...
h[i] =  'MENU BAR:'
h[i] =  'File->Open...:           Read in a new fits image from disk'
i = i + 1
h[i] =  'File->Write FITS...:          Write out a new fits image to disk (single-plane or entire image)'
i = i + 1
h[i] =  'File->Write PS...:            Write a PostScript file of the current display'
i = i + 1
h[i] =  'File->Write Image...:         Write a JPEG, TIFF, BMP, PICT, or PNG image of the current display'
i = i + 1
h[i] =  'File->Get Image:           Download archive images'
i = i + 1
h[i] =  'File->Quit:               Quits GPItv'
i = i + 1
...
h[i] =  'ImageInfo->Contrast:    Calculate contrast using satellite images'
i = i + 1

h[i] =  'ImageInfo->Photometry:    Brings up photometry window'
i = i + 1
h[i] =  'ImageInfo->View FITS Header...:   Display the FITS header, if there is one.'
i = i + 1
h[i] =  'ImageInfo->Change FITS extension...:   Choose another extension, if there is one.'
i = i + 1
...
i = i + 1
h[i] =  'ImageInfo->Archive Image: Dialog to download DSS, 2MASS, or IRAS images into GPItv'
i = i + 1
h[i] =  ''
i = i + 1
h[i] =  'CONTROL PANEL ITEMS:'
i = i + 1
h[i] = 'Min(colorbar):                      Shows minimum data value displayed; enter new min value here'
i = i + 1
h[i] = 'Max(colorbar):                      Shows maximum data value displayed; enter new max value here'
i = i + 1
...
i = i + 1
h[i] =  'Pos/Color:                    Sets color-stretch mode:'
i = i + 1
h[i] = '                            button 1 or 2: center on current position.'
i = i + 1
h[i] = '                            With mouse button 3 down, drag mouse to change the color stretch.  '
i = i + 1
...
h[i] = ' '
i = i + 1
h[i] = 'Stat/ImExam3D:                   Sets ImageExamine mode:'
i = i + 1
h[i] = '                            button 1: Image statistics'
i = i + 1
h[i] = '                            button 2: Center on current position'
i = i + 1
h[i] = '                            button 3: Image 3D statistics'
i = i + 1
h[i] = ' 					Select arbitrary box with two clicks, defining the corners'
i = i + 1
...
h[i] = 'Angprof/Apphot:                 Sets Radial profile and photometry mode:'
i = i + 1
h[i] = '                            button 1: Angular profile'
i = i + 1
h[i] = '                            button 3: Photometry'
i = i + 1
h[i] = 'Spaxel/Lineplot:             Sets Spectrum and Flux vs wavelength mode:  '
i = i + 1
h[i] = '                            button 1: Flux vs wavelength'
i = i + 1
h[i] = '                            button 3: Slight different version'
i = i + 2
...
i = i + 1
h[i] = 'AutoScale:                GPItv AutoScale Min/Max display values for the displayed plane'
i = i + 1
...
i = i + 1
h[i] =  'To pass an array to GPItv:'
i = i + 1
h[i] =  '   GPItv, array_name [, options]'
i = i + 1
h[i] =  ''
i = i + 1
h[i] = 'To pass a FITS filename to GPItv:'
i = i + 1
h[i] =  '   GPItv, fitsfile_name [, options] (enclose filename in single quotes) '
i = i + 1
h[i] =  ''
i = i + 1
...
i = i + 1
h[i]  = '   [,/block] [,/alignp] [,/stretch] [,header=header]'
i = i + 2
h[i] = 'To overplot a contour plot on the draw window:'
i = i + 1
h[i] = '   GPItvcontour, array_name [, options...]'
i = i + 1
h[i] =  ''
i = i + 1
h[i] = 'To overplot text on the draw window: '
i = i + 1
h[i] = '   GPItvxyouts, x, y, text_string [, options]  (enclose string in single quotes)'
i = i + 1
h[i] =  ''
i = i + 1
h[i] = 'To overplot points or lines on the current plot:'
i = i + 1
h[i] = '   GPItvplot, xvector, yvector [, options]'
i = i + 2
h[i] = 'The options for GPItvcontour, GPItvxyouts, and GPItvplot are essentially'
i = i + 1
...
h[i] = 'OTHER COMMANDS:'
i = i + 1
h[i] = '  self->erase [, N]:         Erases all (or last N) plots and text'
i = i + 1
h[i] = '  GPItv_shutdown:           Quits GPItv'
i = i + 1
h[i] = 'NOTE: If GPItv should crash, type GPItv_shutdown at the idl prompt.'
i = i + 5
h[i] = 'Written by Jerome Maire & M. Perrin, GPItv is a modified version of the package ATV written by Aaron Barth.'
...
;endelse
     ...
     if (not (keyword_set(ps)))  then begin
        ...
        if n_elements(indf) gt 1 then $
           plot, xlam, p1d(indf),ytitle='Flux ['+(*self.state).units+']', xtitle='Lambda (um)', psym=-1
     endif else begin
	;indf=where(finite(p1d))
	;if n_elements(indf) gt 1 then $
...
help=strarr(6)
help(0)='  ***  SIMPLE DIFFERENCE IMAGING   ***'
help(1)='Choose predefined wav. intervals in the droplist or enter user-defined values.'
help(2)='Validate new values by pressing the Return or Enter key in the text widget.'
help(3)='It will average slices between wav1 & wav2 (Im1) and between wav3 & wav4 (Im2).'
help(4)='It will scale spatially Im1 by taking into account the ratio of the central wav of the band considered.'
help(5)='After, it will calculate (Im1)-fact*(scaled(Im2)).'
...
;print,(*self.satspots.cens)[0,0,0]
;stop
...
widget_control,(*self.state).sdik_id, GET_VALUE=k
knum=1.
if k ne -1 then knum=double(k)
...
*self.images.main_image=smoothfact*(I1s-knum[0]*I2)
;widget_control,(*self.state).collapse,set_value=4
;(*self.state).collapse=4
...
pro GPItv::anguprof_refresh, ps=ps,  sav=sav
  ...
  if (not (keyword_set(ps))) then begin
     ...
     plot, xx,p1d,ytitle='Flux ['+(*self.state).units+']', xtitle='pixel', psym=-1
     self->resetwindow
...
if (not keyword_set(xrange)) then begin
   ...
   if (*self.state).contr_xunit eq 1 then $
      asec *= 1d/3600d*!dpi/180d*7.7701d/((*(*self.state).CWV_ptr)[inds[0]]*1d-6)
   xrange=[min(asec),max(asec)]
   for j=1,n_elements(inds)-1 do begin
      asec = *(*self.satspots.asec)[inds[j]]
      if (*self.state).contr_xunit eq 1 then $
         asec *= 1d/3600d*!dpi/180d*7.7701d/((*(*self.state).CWV_ptr)[inds[j]]*1d-6)
...
pro GPItv::tvcontr, nosat=nosat, ps3=ps3, nodh=nodh
  ...
  if  not(keyword_set(ps3)) then begin
     ...
     if not(keyword_set(nodh)) then begin
        ...
        if ~strcmp(pix_to_ripple,'ERROR',/fold_case) then pix_to_ripple = double(pix_to_ripple)*lambda/1.5040541d else $
...
pro GPItv::contrprof_refresh, ps=ps,  sav=sav, radialsav=radialsav,forcesat=forcesat
  ...
  if ~(keyword_set(forcesat))  && ((*self.state).collapse eq 0)  then begin
  ...
  if ~(*self.state).contr_autocent then begin
  ...
  ;;filled in yet)
  if ((*self.state).collapse eq 0) || ((*self.state).specalign_mode eq 1) then begin 
     if (*self.state).contr_plotmult then inds = (*self.satspots.good) else $
        inds = (*self.state).cur_image_num
     copsf = (*self.images.main_image_backup)[*,*,inds] ;use backup in case specaligned
  endif else begin
  ...
  for j = 0, n_elements(inds)-1 do begin
     ...
     if  (((*self.state).collapse ne 0) && ((*self.state).specalign_mode ne 1)) || $             ;this is a collapse mode
  ...
  if ((*self.state).collapse eq 0) || ((*self.state).specalign_mode eq 1) then $
...
;;determine initial collapse mode
(*self.state).collapse=0
if N_PARAMS() EQ 0 then  begin
   image = COMMAND_LINE_ARGS( )
   if STRMATCH(STRMID(image,STRLEN(image)-1,1),'%')  then begin
      (*self.state).collapse=1
      image=STRMID(image,0,STRLEN(image)-2)
   endif
endif
...
; MDP change: do not automatically go always to that dir, start in the real 
; current directory.
if ( (n_params() NE 0) AND (size(image, /tname) EQ 'STRING')) then begin
    ...
    if (count EQ 0) then begin
        ...
        self->readfits, fitsfilename=image, alignwav=alignwav
		(*self.state).current_dir=file_dirname(image)
    endelse
endif

if ( (n_params() NE 0) AND (size(image, /tname) NE 'STRING')) then $
 (*self.state).title_extras=SCOPE_VARNAME(image, LEVEL=-1)
 

; MDP note 2008-11-13: Rearranging the code to simplify it and remove some
; redundancies. If there is no image, create one here, then proceed normally.
;   Define default startup image
;if (n_elements(image) LE 1) then begin
if n_params() eq 0 or (n_params() eq 1 and ~(keyword_set(image)) ) then begin

    ;*self.images.main_image = cos(((findgen(500)- 250)*2) # ((findgen(500)-250)))
    ;*self.images.main_image = cos(((findgen(500)- 250)*2.9) # ((findgen(500)-250)*1.5))
    gridsize = 270
    centerpix = 135
	; square pattern:
;;    x = ((findgen(gridsize) # replicate(1, gridsize)) - centerpix + 0.001)*0.5
;;    y = ((replicate(1, gridsize) # findgen(gridsize)) - centerpix + 0.001)*0.5
;;    *self.images.main_image = (sin(x)/x) * (sin(y)/y)
;;    image = (sin(x)/x) * (sin(y)/y)
...
if keyword_set(satspotcalibfile) then (*self.state).satspotcalibfile=satspotcalibfile


IF KEYWORD_SET(smart) THEN (*self.state).smart=1 ; GPItv "smart mode"
...
; If user has passed GPItv a data array, or the data array has been read in
; from disk, then store that data into the *self.images.main_image variable.
;if ( (n_params() NE 0) AND (size(image, /tname) NE 'STRING') AND (size(image, /tname) NE 'UNDEFINED')) then begin
; MDP modification 2008-11-18 to have the default image creation be above this,
; so don't check for n_params() ne 0 here.
continue_setup = 0
if ( (size(image, /tname) NE 'STRING') AND (size(image, /tname) NE 'UNDEFINED')) then begin
   ;; Make sure it's a 2-d or 3-d array
   if ( (size(image))[0] NE 2 AND (size(image))[0] NE 3) then begin
      self->message, msgtype='error', 'Input data must be a 2-d or 3-d array!'
   endif else begin
      if (size(image))[0] EQ 2 THEN begin ; Load a 2D image.
         *self.images.main_image = image
         *self.images.main_image_stack = image
         *self.images.bmask_image = bmask
         (*self.state).image_size = [(size(*self.images.main_image_stack))[1:2], 1]

         if keyword_set(imname) then (*self.state).imagename = imname else $
            (*self.state).imagename=''
         ;;(*self.state).title_extras = ''

         if keyword_set(header) then begin
            if size(header[0],/type) ne 10 then self->setheader, header else self->setheader, *header[0]
         endif else begin
            mkhdr,header,image
         ...
         *self.images.main_image_stack = image
         *self.images.bmask_image_stack = bmask
         if ~keyword_set(alignwav) then (*self.state).cur_image_num = 0
         *self.images.main_image = (*self.images.main_image_stack)[*, *, (*self.state).cur_image_num]
         *self.images.bmask_image = (*self.images.bmask_image_stack)[*,*,(*self.state).cur_image_num]
         (*self.state).image_size = (size(*self.images.main_image_stack))[1:3]
         ...
         if keyword_set(imname) then begin
            *self.images.names_stack = imname
            ...
            (*self.state).imagename=''
            *self.images.names_stack=strarr(n_elements(image[0,0,*]))
         endelse
         if (keyword_set(header)) then begin
            if size(header[0],/type) ne 10 then self->setheader, header else begin
               *self.images.header_stack = header
               header=(*header[0])
            endelse
         endif else begin
            if 0 then for i = 0, (size(image))[3] - 1 do begin
               mkhdr,hdr,image[*,*,i]
               if i eq 0 then outhead=ptr_new(hdr) else $
                  outhead = [outhead,ptr_new(hdr)]
               *self.images.header_stack = outhead
               header = (*outhead[0])
            endfor
         endelse 
         ...
         widget_control,(*self.state).curimnum_base_id0,map=1, xsize=(*self.state).draw_window_size[0], ysize=45
;              xsize=(*self.state).draw_window_size[0]+80,ysize=45
;
;            widget_control,(*self.state).curimnum_base_id,map=1, $
;              xsize=(*self.state).draw_window_size[0]+30,ysize=45
;
         widget_control, (*self.state).curimnum_text_id, sensitive = 1, $
                         set_value = 0
         widget_control, (*self.state).curimnum_slidebar_id, sensitive = 1, $
      ...
      (*self.state).rot_angle = 0.
      (*self.state).invert_image = 'none'
   endelse ;end of 2-d or 3-d array block
   *self.images.main_image_backup = *self.images.main_image_stack
endif ;end of has image size block

;;figure out padding
widget_control, (*self.state).base_id, tlb_get_size=tmp_event
(*self.state).base_pad = tmp_event - (*self.state).draw_window_size

if (continue_setup EQ 1) then begin
   ;; skip this part if new image is invalid or if user selected 'cancel' in dialog box
   self->settitle
   self->setheadinfo
   self->getstats
   self->recenter, align=alignp
   
   *self.images.display_image=0.

   if n_elements(minimum) GT 0 then begin
      (*self.state).min_value = minimum
   endif
   
   if n_elements(maximum) GT 0 then begin
      (*self.state).max_value = maximum
   endif
   ...
   ;;default scaling was already taken care of in GPItv::startup, so
   ;;only process scaling keywords
   ...
   ;; not set
   IF ((*self.state).min_value EQ (*self.state).max_value) OR (keyword_set(stretch) EQ 0) THEN BEGIN
      
      if (keyword_set(autoscale) OR $
          (((*self.state).default_autoscale EQ 1) AND (n_elements(minimum) EQ 0) $
           AND (n_elements(maximum) EQ 0)) ) $
      then self->autoscale
   ENDIF
   self->set_minmax
   
   IF NOT keyword_set(alignp) THEN BEGIN
      (*self.state).zoom_level = 0
      (*self.state).zoom_factor = 2.^(*self.state).zoom_level
   ENDIF
   
   self->changeimage, (*self.state).cur_image_num,/nocheck ; this will display wavelength or Stokes as needed.
   ...
   IF NOT keyword_set(alignp) THEN BEGIN
      if ((*self.state).image_size[0] le (*self.state).draw_window_size[0]) AND $
         ((*self.state).image_size[1] le (*self.state).draw_window_size[1]) then $
            for tempvar = 1, $
                          floor(sqrt(min((*self.state).draw_window_size/(*self.state).image_size[0:1]))) do $
                             self->zoom, 'in'
      if ((*self.state).image_size[0] gt (*self.state).draw_window_size[0]) AND $
         ((*self.state).image_size[1] gt (*self.state).draw_window_size[1]) then $
            for tempvar = 1, $
                          floor(sqrt(min((*self.state).image_size[0:1]/(*self.state).draw_window_size)))+1 do $
                             self->zoom, 'out'
   ENDIF 
endif ;;end continued setup block
