...
function atmos_trans_forpipeline, wind=wind, Lamb=Lamb, airmass=airmass, hdr=hdr
common PIP
;common DST_INPUT
;widthL=(lambda(1)-lambda(0))
mydevice = !D.NAME

fileTransAtmos=getenv('GPI_PIPELINE_DIR')+path_sep()+'dpl_library'+path_sep()+'trans_16_15.dat' ;1.6mm Water vapour column, lambda[um] sampling 5A
ref_airmass = 1.5
...
if ~keyword_set(wind) then wind=0
	if 1 then begin
	  window, wind
