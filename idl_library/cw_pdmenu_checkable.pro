;  This is a modified version of ITT's cw_pdmenu routine, modified to
;  add the ability to check off checkbuttons. This is implemented with 
;  a new bit flag using bit 8.
;
;  See http://www.idlcoyote.com/widget_tips/checkmarks.html
;
;
; Most of the code is
...
;			   be preceeded by a separator.
;
;			 8  This button will be able to have a checkbox marked 
;			 	next to it (i.e. /CHECKED_MENU will be set when creating the
;			 	widget)
;
...
function CW_PDMENU_CHECKABLE_EVENT, ev
...
pro CW_pdmenu_checkable_BUILD, parent, desc, cur, n, ev_type, full_qual_str, $
...
; Returns the ID of each button in ids.
  ...
  while (cur lt n) do begin
    ...
    if is_string then begin
        ...
        if (dflags and 7) GT 3 then begin
          separator = 1
          dflags = dflags - 4
        endif
   ...
   if ((mbars ne 0) and (HELP_KW ne 0) $
    ...
    if (dflags and 1) then $
      CW_pdmenu_checkable_BUILD,new,desc,cur,n,ev_type,$
...
function CW_pdmenu_checkable, parent, desc, COLUMN=column, CONTEXT_MENU=context_menu, $
  ...
  if (not KEYWORD_SET(uname)) then uname = 'CW_pdmenu_checkable_UNAME'
  ...
  WIDGET_CONTROL, base, EVENT_FUNC='CW_pdmenu_checkable_EVENT'

  if ( N_ELEMENTS(tab_mode) ne 0 ) then $
    WIDGET_CONTROL, base, TAB_MODE = tab_mode

 CW_pdmenu_checkable_BUILD, base, desc, 0, n, ev_type, '', delim, ids, mbars, help_kw, $
