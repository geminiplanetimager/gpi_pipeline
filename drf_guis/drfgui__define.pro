...
;+--------------------------------------------------------------------------------
; drfgui::change_current_template
...
;+--------------------------------------------------------------------------------
; drfgui::update_available_primitives
...
;+--------------------------------------------------------------------------------
; drfgui::update_selected_modules
;    Update the list of modules selected for the current Recipe. 
...
;+--------------------------------------
; drfgui::check_output_path_exists
...
;+--------------------------------------
; drfgui::savedrf
;    Save a Receipe (DRF) to an XML file on disk.
;
;    ARGUMENTS:
...
fullfiles=file[where(file NE '')]  
file = file[index]  
drf_summary = self.drf->get_summary()
;print,self.loadedRecipeFile
  ...
  if templatesflag then begin
     ...
     first_file=strsplit(fullfiles[0],'S.',/extract)
...
pro drfgui::loaddrf, filename, nodata=nodata, silent=silent, log=log
    ...
    if ~file_test(filename) then begin
		message, "Requested Recipe file does not exist: "+filename,/info
    ...
    self.drf = obj_new('drf', self.loadedRecipeFile,/silent, /quick)
    drf_summary = self.drf->get_summary()
    ...
    for ii=0,n_elements(drf_module_names)-1 do begin
       matches=where(strmatch(seq,(drf_module_names)[ii],/fold_case), matchct)
       if N_ELEMENTS(matches) gt 1 then begin
          message,/info, "WARNING - Found "+strc(N_ELEMENTS(matches))+" multiple primitives having the same function name - "+ (seq[matches[0]])
       endif else begin
    ...
    CASE !VERSION.OS_FAMILY OF  
           ...
           'Windows'   :begin
              top_base=widget_base(title=title, $
                                   group_leader=groupleader,/BASE_ALIGN_LEFT,/column, MBAR=bar,bitmap=self.dirpro+path_sep()+'gpi.bmp',$
...
;	base_radio = Widget_Base(top_right_advanced_base, UNAME='WID_BASE_diskb', COLUMN=1 ,/NONEXCLUSIVE, frame=0)
	...
	lab = widget_label(base_primitive_args, value="Change values of parameters of the selected primitive [press Enter after each change. Validate new values with ENTER]:")             
        self.tableArgs = WIDGET_TABLE(base_primitive_args, $ ; VALUE=data, $ ;/COLUMN_MAJOR, $ 
                                      ...
                                      COLUMN_WIDTHS=[120,120,120,440],frame=1,/EDITABLE,uvalue='tableargs' , $
	...
	self.textinfoid=widget_label(bot_baseexec,uvalue="textinfo",xsize=900,ysize=10,value='  ')
