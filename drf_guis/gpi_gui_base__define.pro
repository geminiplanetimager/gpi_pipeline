...
;
;	Any derived classes should implement the following procedures:
;
;		init_data
;		init_widgets
;
...
;
; HISTORY:
;   2012-08-17  Started by Marshall Perrin, refactored from drfgui__define.
;------------------------------------------------
...
pro gpi_gui_base::confirm_close
	if confirm(group=self.top_base,message='Are you sure you want to close the '+self.name+' window?',$
...
pro gpi_gui_base__define
    struct = {gpi_gui_base,   $
			  ...
			  menubar: obj_new(),$		; object reference for menu bar object (see checkable_menu__define)
			  widget_log: 0L, $
              dirpro:'',$
              queuedir:'',$
              inputdir:'',$
