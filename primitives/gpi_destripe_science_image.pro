;+
; NAME: gpi_destripe_science_image
...
;////////////////////////////////////////////////////////////////////////////////
  ...
  if (remove_microphonics GE 1) then begin
    ...
    ;If the algorithm is applied based on the threshold, load the threshold or get a default value
    ;Else threshold set to zero to be sure the algo will be applied
    if (remove_microphonics eq 2) then begin
        if tag_exist( Modules[thisModuleIndex], "micro_threshold") then micro_threshold = float(Modules[thisModuleIndex].micro_threshold) else begin
          micro_threshold = 0.01
          backbone->Log, "Parameter micro_threshold not found. Default value = 0.01",depth=2
          backbone->set_keyword, "HISTORY", "Parameter micro_threshold not found. Default value = 0.01"
        endelse
    endif else begin
        micro_threshold = 0.0
    endelse
        ...
        if (noise_before  GE micro_threshold) then begin
...
;////////////////////////////////////////////////////////////////////////////////
  ...
  ; /////////////////////////////////////////
    full_noise_model=stripes
    if keyword_set(chan_offset_correction) then full_noise_model+=chan_offset
