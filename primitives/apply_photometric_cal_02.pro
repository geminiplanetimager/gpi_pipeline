...
; PIPELINE PRIMITIVE DESCRIPTION: Calibrate Photometric Flux and save conversion in DB 
;
;
; INPUTS: data-cube
;
...
;
; OUTPUTS:  Flux calibrated datacube
;
...
;;; handle the spot locations
 SPOTWAVE=backbone->get_keyword('SPOTWAVE',  COUNT=cc4)
   if cc4 gt 0 then begin
    ...
    apr = 2.*(lambda[n_elements(lambda)/2]*1.e-6/gpi_get_constant('primary_diam',default=7.7701d0))*$
          (180.*3600./!dpi)/0.014                      ;[radaper];lambda[0]*[3.];lambda[0]*[5.];lambda[0]*[3.] 
