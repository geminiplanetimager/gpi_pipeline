;+
; NAME: Extract_telluric_transmission_meth1
; PIPELINE PRIMITIVE DESCRIPTION: Extract telluric transmission from sat. spots
;
;
;
; INPUTS: 
...
; OUTPUTS:  
;
; PIPELINE COMMENT: Extract Telluric Spectrum from satelitte spots
...
function extract_telluric_transmission_meth1, DataSet, Modules, Backbone
...
inputdcspot1=dblarr(CommonWavVect[2])
inputdcspot2=dblarr(CommonWavVect[2])
fluxsatabs=dblarr(CommonWavVect[2],2)
fluxsatmedabs=dblarr(CommonWavVect[2])

;;TODO:location of spots should come from elsewhere
x=uint(Modules[thisModuleIndex].Xspot1) & y=uint(Modules[thisModuleIndex].Yspot1);spot1 location
for i=0,CommonWavVect[2]-1 do begin
    ...
    inputdcspot1[i]=(flux[0]-sky[0]);/(nbphot[i]   ) 
endfor

x=uint(Modules[thisModuleIndex].Xspot2) & y=uint(Modules[thisModuleIndex].Yspot2);spot2 location
for i=0,CommonWavVect[2]-1 do begin
    aper, cubcent2[*,*,i], [x], [y], flux, errap, sky, skyerr, phpadu, apr, $
    skyrad, badpix, /flux, /silent ;, flux=abs(state.magunits-1)
    print, 'slice#',i,' flux sat2=',flux[0],' sky=',sky[0]
    inputdcspot2[i]=(flux[0]-sky[0]);/(nbphot[i]  )
endfor

fluxsatabs[*,0]=inputdcspot1
fluxsatabs[*,1]=inputdcspot2
for i=0,CommonWavVect[2]-1 do fluxsatmedabs[i]=mean(fluxsatabs[i,*],/nan)
...
if tag_exist( Modules[thisModuleIndex], "Save_corrected_datacube") && tag_exist( Modules[thisModuleIndex], "suffix") then suffix+=Modules[thisModuleIndex].suffix
    ...
    if tag_exist( Modules[thisModuleIndex], "Save_corrected_datacube") && ( Modules[thisModuleIndex].Save_corrected_datacube eq 1 ) then begin
      ...
      if tag_exist( Modules[thisModuleIndex], "gpitv") && ( fix(Modules[thisModuleIndex].gpitv) ne 0 ) then $
          gpitv, double(*DataSet.currFrame), ses=fix(Modules[thisModuleIndex].gpitv),head=*(dataset.headers)[numfile]
    endelse
