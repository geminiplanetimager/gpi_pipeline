...
;stop
    ...
    for spot=1,(size(spotloc))[1]-1 do begin
      ...
      pos2=fltarr(2,CommonWavVect[2])
        for i=0,CommonWavVect[2]-1 do begin
            ;;extrapolate sat -spot at a given wavelength
            pos2[*,i]=calc_satloc(spotloc[spot,0],spotloc[spot,1],spotloc[0,*],SPOTWAVE,lambda[i])
            
            pos2b=pos2[0,i]
            pos2c=pos2[1,i]
            pos2[0,i]=pos2c
            pos2[1,i]=pos2b
              x=long(ceil(pos2[0,i]))
              y=long(ceil(pos2[1,i]))
              ...
              if i eq 0 then firstpos=pos2[*,i]
              if i eq CommonWavVect[2]-1 then lastpos=pos2[*,i]
              if i eq floor((CommonWavVect[2])/2) then centersat=pos2[*,i]
...
;        stop  
              ...
              distance=sqrt((firstpos[0]-lastpos[0])^2.+(firstpos[1]-lastpos[1])^2.)+4.
              disttab=distarr(sz,sz,centersat[0],centersat[1])
              annulus=where(disttab ge (distance) AND disttab le (distance)+2)
              photom_annulus=total(StokesI[annulus[*]]) /float(n_elements(annulus))
              photomsat-=photom_annulus*float(n_elements(pos1))
              print, ' flux sat #',spot,'=',photomsat,' sky=',photom_annulus
              aper, abs(stokesI), centersat[0],centersat[1], flux, errap, sky, skyerr, phpadu, distance/2., $
            skyrad, badpix, /flux, /silent
            print, 'Adopted flux sat #',spot,'=',flux,' sky=',sky
      ;  skyrad[1]+=1.
      ; endwhile
...
convfac=(total(nbphotnormtheosmoothed)/(gaindetector*mean(gridratio)*(fluxsatmedabs)))
...
;convert datacube from IFSunits  to [photons/s/nm/m^2]
        for i=0,(size(cubef3d))[3]-1 do begin
          cubef3D[*,*,i]*=double(convfac)
        endfor
...
unitslist = ['Counts', 'Counts/s','ph/s/nm/m^2', 'Jy', 'W/m^2/um','ergs/s/cm^2/A','ergs/s/cm^2/Hz']
 ...
 ; let's the user define what will be the final units:
      ...
      case unitschoice of
      0: begin ;'Counts'
        for i=0,(size(cubef3d))[3]-1 do cubef3D[*,*,i]/=(float(convfac)/float(exposuretime))
      end
      1:begin ;'Counts/s'
        for i=0,(size(cubef3d))[3]-1 do cubef3D[*,*,i]/=(float(convfac))
        end
      2: begin ;'ph/s/nm/m^2'
         end
      3:  begin ;'Jy'
        for i=0,(size(cubef3d))[3]-1 do begin
          cubef3D[*,*,i]*=(1e3*(lambda[CommonWavVect[2]/2])/1.509e7)
        endfor
        end
      4:  begin ;'W/m^2/um'
        for i=0,(size(cubef3d))[3]-1 do begin
          cubef3D[*,*,i]*=(1.988e-13/(1e3*(lambda[CommonWavVect[2]/2])))
        endfor
        end
      5:  begin ;'ergs/s/cm^2/A'
        for i=0,(size(cubef3d))[3]-1 do begin
        cubef3D[*,*,i]*=(1.988e-14/(1e3*(lambda[CommonWavVect[2]/2])))
        endfor
        end
      6:  begin ;'ergs/s/cm^2/Hz'
        for i=0,(size(cubef3d))[3]-1 do begin
        cubef3D[*,*,i]*=((1e3*(lambda[CommonWavVect[2]/2]))/1.509e30)
        endfor
        end
      endcase
