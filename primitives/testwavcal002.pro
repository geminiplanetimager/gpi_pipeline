;+
; NAME: testwavcal
; PIPELINE PRIMITIVE DESCRIPTION: Test the wavelength solution measurement with vertical dispersion axis
;
; INPUTS: 
;
;
...
function testwavcal002, DataSet, Modules, Backbone
primitive_version= '$Id: testwavcal002.pro 11 2010-08-09 10:22:03 maire $' ; get version from subversion to store in header history
@__start_primitive
...
;;; First, if not already done, need to format the DST Zemax file as a DRP wavelength solution
  ...
  if (Modules[thisModuleIndex].CalibrationFile eq '') then begin
    ...
    zemdispYraw=readfits(rep+'zemdispY'+filter+'.fits') ;readfits(rep+'zemdispX'+filter+'.fits')
    zemdispXraw=readfits(rep+'zemdispX'+filter+'.fits') ;readfits(rep+'zemdispY'+filter+'.fits')
    ;actually the raw zemax files has a larger lenslet array, so adjust the size for the comparison 
 
 ;;;;;;;;;;;;;;;;;;;;;;;;
    zemdispY = -zemdispYraw ; for some reason this appears necessary, a vertical flip
    zemdispX = -zemdispXraw ; for some reason this appears necessary, a vertical flip

    ; the spectra are not truly vertical at the center, they are rotated by
    ; a tiny amount. Looking at one H band Xe frame, it looks like the
    ; run/rise = 4/140 pixels (roughly) = 1.63 degrees
    run=-8.
    rise=290.


    rotangle=run/rise
    zemdispXp =  cos(rotangle)*zemdispX + sin(rotangle)*zemdispY
    zemdispYp = -sin(rotangle)*zemdispX + cos(rotangle)*zemdispY
;    zemdispXp =  zemdispX 
;    zemdispYp =  zemdispY
filter="H"
    case filter of
    'H': offsets=[-1., -4.]
      ...
      message,/info, "*** no empirical offsets yet defined for that filter! ***"
      offsets=[0,0]
    end
    endcase


    zemdispX = zemdispXp +offsets[0]
    zemdispY = zemdispYp +offsets[1] ; empirical offsets applied after rotation to align them.

  zemdispx2 = 1024.+zemdispx;transpose(zemdispx, [1,2,0])  ; by putting the have the largest iteration's accessed axis (X) last
  zemdispy2 = 1024.+zemdispy;transpose(zemdispy, [1,2,0])
  ...
  ;;;;;;;;;;;;;;;;;;
   ...
   szzem=size(zemdispXraw)
;   zemdispY=zemdispYraw+1020.
;   zemdispX=zemdispXraw+1020.
       ;we need to have the same origin and edges
    ; zemdispX2=zemdispX+1020. ;rotate(zemdispX,2)+1020.
    ; zemdispY2=zemdispY+1023.  ;rotate(zemdispY,2)+1023.
    
;    zemdispY2=fltarr(nlens,nlens,szzem[3])
;    zemdispX2=fltarr(nlens,nlens,szzem[3])
;    for zl=0,szzem[3]-1 do zemdispY2[*,*,zl]=rotate(zemdispY[*,*,zl],2)
;    for zl=0,szzem[3]-1 do zemdispX2[*,*,zl]=rotate(zemdispX[*,*,zl],2)
     ...
     for i=0,nlens-1 do begin
      for j=0,nlens-1 do begin
        ...
        for p=0,n_elements(zemdisplamraw)-1 do begin
          tiltp[p]=-atan((zemdispX2[i,j,p]-zemdispX2[i,j,zemwavind])/(zemdispY[i,j,p]-zemdispY[i,j,zemwavind]))
          w3p[p]=abs(zemdisplamraw[p]-zemdisplamraw[zemwavind])/(sqrt(((zemdispX[i,j,p]-zemdispX[i,j,zemwavind]))^2+(zemdispY[i,j,p]-zemdispY[i,j,zemwavind])^2))
...
;          zemdispY2[where(zemdispY2 gt 2040.)]=!VALUES.F_NAN  
          zemdispX3=zemdispX2[*,*,zemwavind]
          zemdispY3=zemdispY2[*,*,zemwavind]
          nanindx=where(~finite(wavcal2[*,*,1]))
          nanindy=where(~finite(wavcal2[*,*,0]))
             zemdispX3[nanindx]=!VALUES.F_NAN   
             zemdispY3[nanindy]=!VALUES.F_NAN 

    ;creation de la zem-wavcal au format DRP
    szwcdrp=size(wavcal1)
    zemwavcal=fltarr(szwcdrp[1],szwcdrp[2],szwcdrp[3])
    zemwavcal[*,*,0]=zemdispy3 ;rotate((zemdispY2[*,*,zemwavind]),2)+4. ;rotate(transpose(zemdispX2[*,*,zemwavind]),2)+4.
    zemwavcal[*,*,1]=zemdispx3 ;rotate((zemdispX2[*,*,zemwavind]),2)+4. ;rotate(transpose(zemdispY2[*,*,zemwavind]),2)+4.
    zemwavcal[*,*,2]=zemdisplamraw[zemwavind]
    zemwavcal[*,*,3]=w3 ;rotate(transpose(w3),2)
    zemwavcal[*,*,4]=tilt ;rotate(transpose(tilt),2)
...
gpitv, wavcal2-zemwavcal,/block ; & gpitv_activate
;stop
 ...
 for zind=deb,n_elements(zemdisplamraw)-1-deb do begin
  wavcalz=change_wavcal_lambdaref( wavcal1, zemdisplamraw[zind])
    zemwavcalz=fltarr(szwcdrp[1],szwcdrp[2],szwcdrp[3])
             zemdispX3=zemdispX2[*,*,zind]
             zemdispY3=zemdispY2[*,*,zind]
             
             zemdispX3[nanindx]=!VALUES.F_NAN   
             zemdispY3[nanindy]=!VALUES.F_NAN 
     zemwavcalz[*,*,0]= zemdispY3
     zemwavcalz[*,*,1]= zemdispX3
