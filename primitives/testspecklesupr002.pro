;+
; NAME: testspecklesupr002
; PIPELINE PRIMITIVE DESCRIPTION: Test the speckle suppression algorithms, plots
;
; INPUTS: 
;
;
...
print, 'fwhm=',fwhm
gaus = PSF_GAUSSIAN( Npixel=3*fwhm, FWHM=1.*fwhm, NDIMEN =1, /NORMAL )
...
val=sxpar( *(dataset.headers[0]),"AVPARANG",count=ck)
...
for ii=0,nbcomp-1 do begin
  ...
  ;;;;photometric measurement of the comp
    ...
    radi=4. & apr = (1./2.)*lambda[0]*float(radi)
...
;espe=extr[*,2]
COMPMAG=compmagh[ii]
print, 'spec[0]=',(10.^(-(compmag-refmag)/2.5))*spec[0]
...
expo=17 ;floor(abs(alog10(maxvalue))+1.)
factorexpo=10.^expo 
thisLetter = "155B
greekLetter = '!9' + String(thisLetter) + '!X'
print, greekLetter
units=' W/m^2/'+greekLetter+'m'
extra=Modules[thisModuleIndex].title
title=strcompress(SXPAR( header, 'SPECTYPE'),/rem)+' star, Exposure='+strcompress(strc(sxpar(header,'EXPTIME'), format='(f6.1)'),/rem)+'s, ';+Modules[thisModuleIndex].title
if ii gt 0 then numcomp='comp'+strc(floor(ii)) else numcomp=''
s_Ext='-compn_x'+strc(floor(x))+'_y'+strc(floor(y))+numcomp
...
if (size(suffix))[1] eq 0 then suffix=''
   ...
   if numfile eq 0 then   psFilename = Modules[thisModuleIndex].OutputDir+'figm'+path_sep()+strmid(filnm, 0,strlen(filnm)-5)+suffix+s_Ext+'.ps'
   
    if total(finite(espe)) gt 30 then begin
    if numfile ne 0 then set_plot,'ps'
   if numfile eq 0 then openps,psFilename, xsize=17, ysize=14 ;, ysize=10, xsize=15
;      !P.MULTI = [0, 1, 3, 0, 0] 
    units=TeXtoIDL(" W/m^{2}/\mum")
    deltaH=TeXtoIDL(" \DeltaH=")
    comp=TeXtoIDL("H_{comp}=")
    star=TeXtoIDL("I_{star}=")
    print, 'units=',units
    expostr = TeXtoIDL(" 10^{-"+strc(expo)+"}")
   if numfile eq 0 then plot, ewav, factorexpo*espe,ytitle='Flux density ['+expostr+units+']', xtitle='Wavelength (' + greekLetter + 'm)',$
     xrange=[ewav[0],ewav[n_elements(ewav)-1]],yrange=[0,10.],linestyle=2, charsize=1.5;, title=title 
   if numfile eq 1 then oplot, ewav, factorexpo*espe, linestyle=1
   if numfile eq 2 then oplot, ewav, factorexpo*espe, linestyle=3
 ...
 errcurr=mean(100.*abs(espe[indfini]-theospectrum[indfini])/theospectrum[indfini])
  save,errcurr,filename=Modules[thisModuleIndex].OutputDir+strc(numfile)+'temp.sav' 
 if numfile eq 2 then begin
 err=fltarr(3)
  for jj=0,2 do begin 
    restore, filename=Modules[thisModuleIndex].OutputDir+strc(jj)+'temp.sav'
     err[jj]=errcurr
   endfor
  endif
   if numfile eq 2 then legend,['input '+strcompress(compspec,/rem)+' spectrum, '+comp+strc(compmag,format='(f5.1)')+', separation[as]='+strc(compangsep[0], format='(f5.1)'),$
   'No Speckle Sup., Err='+strc(err[0], format='(f6.2)')+'%', 'LOCI Err='+strc(err[1], format='(f5.2)')+'%','LOCI-PSF Err='+strc(err[2], format='(f5.2)')+'%'],linestyle=[0,2,1,3]
...
if numfile eq 2 then xyouts, ewav[1],7.4,star+strc(starmag,format='(f5.1)')+','+title+extra
    if numfile eq 2 then closeps
    set_plot,'win'
    endif
