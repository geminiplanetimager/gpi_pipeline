;+
; NAME: testspecklesuprsdi001
; PIPELINE PRIMITIVE DESCRIPTION: Test the speckle suppression algorithms of SDI.
;
; INPUTS: 
;
;
...
function testspecklesuprsdi001, DataSet, Modules, Backbone
primitive_version= '$Id: testspecklesuprsdi001.pro 11 2010-09-14 10:22:03 maire $' ; get version from subversion to store in header history
@__start_primitive
mydevice = !D.NAME
...
repDST=gpi_get_directory('GPI_DST_DIR')+path_sep()
...
if (size(suffix))[1] eq 0 then suffix=''
     ...
     ;psFilename = Modules[thisModuleIndex].OutputDir+'fig'+path_sep()+strmid(filnm, slash,strlen(filnm)-5-slash)+s_Ext+'.ps'
      psFilename = Modules[thisModuleIndex].OutputDir+'test11_'+strmid(filnm, 0,strlen(filnm)-5)+suffix+s_Ext+'.ps'
...
;print, 'Atten vs rad (moy sur lambda)=',attenuationrmoy
 slash=strpos(filnm,path_sep(),/reverse_search)
 if strmatch(filnm,'.fits') then filnm=strmid(filnm, slash,strlen(filnm)-5-slash) else filnm=strmid(filnm, slash,strlen(filnm)-slash)
psFilenameSNR2 =Modules[thisModuleIndex].OutputDir+filnm+suffix+'SDIATTEN2.ps'
